...
#include "video_privacy/image_preprocessor.hpp"
...
struct FaceDetector::Impl
{
    explicit Impl(
        ...
        : config(std::move(detectorConfig)),
          environment(ORT_LOGGING_LEVEL_WARNING, "video_privacy"),
          session(nullptr)
    {
        ...
        if (config.useCuda && providerIsAvailable("CUDAExecutionProvider"))
        {
            try
            {
